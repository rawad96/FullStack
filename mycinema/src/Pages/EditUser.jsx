...
const EditUser = () => {
    ...
    const [date, setdate] = useState(sessionStorage['date'])
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setuser(data.find(u => u._id === sessionStorage['id']))
        }
        fetchData()
    }, [])
    ...
    const updateUSer = async () => {
        ...
        try {
            const senddata = await axios.put(`${usersurl}/${user._id}`, obj)
            const senddata1 = await axios.put(`${permissionsurl}/${user._id}`, obj2)
            console.log(senddata1);
            const senddata2 = await axios.put(`${userurl}/${user._id}`, { username: UserName, password: user.password })
            navigate('/main/manageusers/allusers')
        } catch (err) {
            ...
            <Container>
                <Card>
                    ...
                    <Card.Body>
                        ...
                        <Row className="mt-2">
                            <Button type="submit" style={{ width: "20%", marginLeft: "29%" }} onClick={updateUSer}>Save</Button>
                            <Button type="button" style={{ width: "20%", marginLeft: "2%" }} onClick={() => navigate('/main/manageusers/allusers')}>Cansel</Button>
                        </Row>
                    </Card.Body>
                </Card>
            </Container>
